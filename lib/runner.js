...
var Runner = exports.Runner = function () {
  ...
  self.runEachSpec = function (spec_group) { 
    var Async_Handler = function (specification) {
            var self = this;
            var self.active_asyncs = 0;
            
            self.async_wait = function (num_of_async_calls) {
                if (num_of_async_calls) {
                  active_asyncs =   num_of_async_calls;
                } else {
                    active_asyncs = 1;
                };
            },
            
            self.async_done = function () {
                  active_asyncs = active_asyncs - 1;
            };        
        };
    ...
    spec_group.specs.forEach( function (spec) {
        ...
        try {
            spec.fn.call(async_handler);
            if (async_handler.active.asyncs === 0) {
                spec.passed = true;    
            } else {
                setTimeout(function () {
                   if ( async_handler.active.asyncs !== 0) {
