...
var matcher = function (actual) {
  var actual = actual, self = actual;
    ...
    this.beLessThan = function (expected) {
      if (actual > expected) {
       throw { message: actual + " is greater than " + expected + " expected to be less"}   
      }
    };

    this.beGreater = function (expected) {
      if ( actual < expected ) {
        throw { message: actual + "is smaller than " + expected + " expecting greater than"}
      }
    };
